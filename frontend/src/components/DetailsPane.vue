...
const fileActivity = computed(() => {
 ...
 if (!key) return [];
 return store.activity.filter((x) => (x.detail || '').includes(key)).slice(0, 5);
});
function copyLink() {
 const f = props.file;
 if (!f) return;
 const q = f.profileId ? `?profileId=${f.profileId}` : '';
 navigator.clipboard.writeText(`${location.origin}/api/download/${encodeURIComponent(f.hash)}${q}`);
 toast('Link download disalin', 'ok');
}
function copyHash() {
...
<template>
 ...
 <div v-for="a in fileActivity" :key="a.id" class="text-[12px] p-2.5 rounded border" :style="{ borderColor: 'var(--border)' }">
 <div class="font-medium" :style="{ color: 'var(--text)' }">{{ a.action }}</div>
 <div :style="{ color: 'var(--text-dim)' }">{{ a.detail }}</div>
 </div>
 <div v-if="!fileActivity.length" class="text-[12px] text-center py-4" :style="{ color: 'var(--text-dim)' }">No activity for this file</div>
 </div>
 </div>
 </div>
 </aside>
