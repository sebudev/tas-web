...
<template>
 ...
 <button class="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-[6px] text-[13px] border bg-white dark:bg-[#262626] hover:bg-[var(--bg)] md:hidden" :style="{ borderColor: 'var(--border)', color: 'var(--text-dim)' }" @click="emit('toggle-folders')"><FolderPlus :size="14" /></button>
 ...
 <button v-for="t in [{v:'all',l:'All'},{v:'image',l:'Image'},{v:'video',l:'Video'},{v:'audio',l:'Audio'},{v:'doc',l:'Doc'},{v:'archive',l:'Archive'}]" :key="t.v" class="px-2 py-1 rounded-full text-[11px] font-medium border" :style="store.filterType===t.v ? { background: 'var(--text)', color: 'var(--card)', borderColor: 'var(--text)' } : { background: 'var(--card)', color: 'var(--text-dim)', borderColor: 'var(--border)' }" @click="store.filterType=t.v; applyFilters();">{{t.l}}</button>
 </div>
 ...
 <input v-model="store.search" class="w-full sm:w-[240px] pl-7 pr-3 py-1.5 rounded-[6px] border bg-[var(--bg)] dark:bg-[#1F1F1F] text-[13px] outline-none focus:bg-white dark:focus:bg-[#262626] focus:border-[#2383E2]" :style="{ borderColor: 'var(--border)', color: 'var(--text)' }" type="search" placeholder="Search…" @input="onSearchInput" />
 <span class="absolute left-2 top-1/2 -translate-y-1/2 opacity-40 pointer-events-none"><Search :size="13" /></span>
 </div>
 <SelectRadix class="shrink-0" :model-value="store.sort" :options="sortOptions" title="Urutkan" @update:model-value="onSort" />
 </div>
 </div>
</template>
